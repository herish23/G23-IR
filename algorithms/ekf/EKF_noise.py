...
class CFG:
    ...
    _experiment_cfg = exp_cfg.get("experiment", {})
    if isinstance(_experiment_cfg, dict):
        _name = _experiment_cfg.get("name")
        if _name:
            OUT_TAG = str(_name)

    OUT_DIR = exp_cfg.get("output_dir", OUT_DIR)
    RANDOM_SEED = exp_cfg.get("random_seed", None)

    Q_v = float(_odom_cfg.get("noise_std", 0.01))
    Q_w = float(_odom_cfg.get("noise_std", 0.01))

    R_sigma = float(_lidar_cfg.get("noise_std", 0.02))

    sparsity = int(_lidar_cfg.get("sparsity", 1))
    BEAM_STRIDE = max(1, sparsity)
    MAX_RANGE = float(_lidar_cfg.get("max_range", 15.0))
    MIN_RANGE = 0.02
    ...
    INIT_P = np.diag([sigma_x0**2, sigma_y0**2, sigma_th0**2])
...
def load_ros_yaml(yaml_path):
    ...
    return img, res, origin, occ_t,


def load_pgm(img_path) -> np.ndarray:
    img = Image.open(img_path).convert("L")
    return np.asarray(img, dtype=np.float32) / 255.0


def make_ogm_from_gray(gray: np.ndarray, occ_thresh: float) -> np.ndarray:
    return (gray <= (1.0 - occ_thresh)).astype(np.uint8)
...
def load_map_and_dt() -> MapDT:
    ...
    dt = edt_distance_transform(occ, res)
    gx, gy = gradient_central(dt, res)

    return MapDT(occ=occ, dt=dt, gx=gx, gy=gy, res=res, origin=origin)
...
def generate_visualization(results: List[Dict[str, str]], output_file: str, mx: MapDT):
    ...
    print("[EKF-OGM] Trajectory video saved")
...
def main():
    ...
    for k, row in enumerate(rows_in):
        ...
        for _ in range(CFG.MAX_UPD_ITERS):
            x, P = implicit_update_once(x, P, mx, ranges, ang_min, ang_inc)
