...
def parse_row_ranges(row: Dict[str, str]) -> Tuple[np.ndarray, float, float, float]:
    """
    Raises have tried to fix the number of columns at 359.
    but, the number of (csv) is lack.
    The missing columns can only be replaced by the maximum value.
    """
    if CFG.CSV_HAS_RANGES_JSON:
        ...
        lidar_cols = [k for k in row.keys() if k.startswith(CFG.R_PREFIX)]
        if not lidar_cols:
            ...
            if not lidar_indices:
                ranges = np.zeros(0, dtype=np.float32)
                ang_min = -np.pi
                ang_inc = 2 * np.pi / 360
            else:
                n_beams = 360
                scan = np.full(n_beams, CFG.MAX_RANGE, dtype=np.float32)
                for col, idx in zip(lidar_cols, lidar_indices):
                    if 0 <= idx < n_beams and row[col] != "":
                        scan[idx] = float(row[col])
                ranges = scan
                ang_min = -np.pi
                ang_inc = 2 * np.pi / n_beams

    dt = float(row.get("dt", "nan"))
    if not np.isfinite(dt):
        dt = CFG.DT_DEFAULT

    if CFG.BEAM_STRIDE >= 2 and ranges.size > 0:
